...
class MLService:
    ...
    def predict(self, feature_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Returns clamped and validated prediction object.
        """
        if not self.loaded:
            self.load_models()
        ...
        min_cell_v = float(feature_dict.get("min_cell_voltage_V", 3.4))
        pack_v = float(feature_dict.get("pack_voltage_V", 11.0))
        imbalance_v = float(feature_dict.get("cell_voltage_imbalance_V", 0.05))
        temp_c = float(feature_dict.get("avg_temperature_C", 27.0))

        if min_cell_v <= 0.5 or pack_v <= 1.0:
            predicted_soc = 0.0
            system_status = "CRITICAL"
        elif min_cell_v <= 2.5 or imbalance_v >= 0.35 or temp_c >= 55.0 or predicted_anomaly != "normal":
            system_status = "CRITICAL" if (min_cell_v <= 2.0 or predicted_anomaly in ["critical_failure", "thermal_runaway"]) else "ANOMALY"
        elif imbalance_v >= 0.15 or temp_c >= 45.0 or predicted_soh < 80.0:
